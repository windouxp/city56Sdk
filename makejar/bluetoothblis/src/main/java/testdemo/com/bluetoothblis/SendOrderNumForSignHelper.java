...
public class SendOrderNumForSignHelper {
    ...
    public SendOrderNumForSignHelper(Context c, BluetoothResultListener bluetoothResult, String ordernum) {
        this.c = c;
...
//        }
    ...
    public void sendOrderNumForSign(final String code) {
    ...
    private void no11111() {

        Observable.timer(1, TimeUnit.MILLISECONDS).observeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new Action1<Long>() {
            @Override
            public void call(Long aLong) {
            ...
            public void call(Long aLong) {
                bleManager.notify(seuiec, t1111, new BleCharacterCallback() {
                    ...
                    public void onSuccess(BluetoothGattCharacteristic characteristic) {
                        ...
                        Log.d(TAG, "onSuccess: 监听dd88的 :" + str.toString());
                        if (str.toString().startsWith("dd88") && str.toString().endsWith("ef")) {
...
//                                rc61();
  ...
  private  void raaaaaa59() {
        ...
        Observable.timer(500, TimeUnit.MILLISECONDS).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new Action1<Long>() {
            @Override
            public void call(Long aLong) {
                bleManager.writeDevice(seuiec, t1111, ByteUtils.hexString2Bytes("AA551C1234AB60" + ordernum + "05EF"), new BleCharacterCallback() {
                    @Override
                    public void onSuccess(BluetoothGattCharacteristic characteristic) {
                    ...
                    public void onSuccess(BluetoothGattCharacteristic characteristic) {
                        ...
                        if (ByteUtils.bytes2HexString(b).startsWith("dd66") && ByteUtils.bytes2HexString(b).endsWith("ef")) {
                            if (b[3] == 0x01) {
                                ...
                                bluetoothResult.returnSignDate(time);
                                bluetoothResult.onSendOrderForSignSuccess();
//                                if(str.substring(8,12).equals())
